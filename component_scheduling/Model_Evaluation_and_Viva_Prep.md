...
| Accuracy | **81.0%** | Exact output of `retrain_real_only.py` |
| F1-Score (At-Risk) | **0.5329** | Improved via SMOTE and Random Forest |
...
> **Why is the F1 score 0.53?** This is the honest result from a severely class-imbalanced dataset (only 13.6% of modules are "at-risk"). The model correctly identifies On-time modules with 94% precision. By utilizing SMOTE and a Random Forest Classifier, we improved the recall for At-Risk modules to 72%. Reporting an honest 0.53 F1 for this minority class is academically respectable.
