...
export default function TopicFeasibilityPanel({ workbookFile, selectedSolution, studentsPerTeam }) {
  ...
  const sourceTeam =
    teams.find((team) => team.project_id === sourceProjectId) || teams[0] || null;
  const targetProject =
    teams.find((team) => team.project_id === targetProjectId) || teams[0] || null;
  ...
  const analyzeFeasibility = async () => {
    ...
    if (!sourceTeam || !targetProject) {
      setErrorMessage('Please select a team and topic.');
      return;
    }
    ...
    try {
      ...
      console.error('Topic feasibility failed:', error);
  ...
  return (
    <section className="mt-8 bg-slate-800/80 border border-slate-700 rounded-xl p-6">
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-white">Topic Technical Feasibility</h2>
        <p className="text-sm text-slate-400 mt-2 max-w-3xl">
          Evaluate an existing team against the modeled technical requirements of an
          approved project or topic. This analysis measures technical coverage only
          and does not predict overall project success.
        </p>
      </div>
      ...
      <div className="mt-6 flex flex-wrap items-center gap-4">
        ...
        >
          {analyzing ? 'Analyzing...' : 'Analyze Technical Feasibility'}
        </button>

        {sourceTeam?.project_id === targetProject?.project_id && (
          <p className="text-xs text-slate-500">
            Evaluating the team against its currently assigned project.
          </p>
        )}
      </div>
...
function FeasibilityResult({ result }) {
  const gapRequirements = result.requirements.filter(
    (requirement) => requirement.status === 'Gap'
  );
    ...
    <div className="mt-8">
      ...
      {!result.team.team_size_matches_project && (
        <div className="mb-6 border border-amber-500/40 bg-amber-500/10 rounded-lg p-4">
          ...
          <p className="text-sm text-slate-300 mt-1">
            This project expects {result.team.expected_team_size} members, but the
            selected team currently contains {result.team.actual_team_size}.
          </p>
          <p className="text-xs text-slate-400 mt-2">
            Technical coverage is still calculated for the current team. This result
            does not mean that the project's team size requirement has been satisfied.
          </p>
        </div>
      )}
      ...
      <div className="mt-7 border border-slate-700 bg-slate-900/60 rounded-lg p-4 text-xs text-slate-400">
        {result.interpretation.scope_note}
      </div>
    </div>
  );
}
