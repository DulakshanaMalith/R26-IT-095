...
def validate_assignment(
    ...
    assignment_projects = set(assignment.keys())
    expected_project_ids = set(expected_projects.keys())

    missing_projects = expected_project_ids - assignment_projects
    unknown_projects = assignment_projects - expected_project_ids
    ...
    for project_id, team in assignment.items():
        expected_size = expected_projects[project_id]

        if len(team) != expected_size:
            raise ValueError(
                f"Project '{project_id}' requires "
                f"{expected_size} students, but received {len(team)}."
            )
    ...
    if len(assigned_students) != len(set(assigned_students)):
        raise ValueError(
            "A student appears more than once in the assignment."
        )

    missing_students = valid_students - set(assigned_students)

    if missing_students:
        raise ValueError(
            "The following students are not assigned: "
...
def calculate_project_technical_coverage(
    ...
    if not requirements:
        raise ValueError(
            f"Project '{project_id}' has no technical requirements."
        )
    ...
    for requirement in requirements:
        ...
        for student_id in team:
            ...
            skill_level = student.skills.get(
                requirement.technology,
                0,
            )

            if skill_level >= requirement.min_level:
                qualified_students.append(student_id)

        qualified_count = len(qualified_students)

        coverage = min(
            1.0,
            qualified_count / requirement.required_members,
        )
        ...
        requirement_results.append(
            {
                ...
                "qualified_members": qualified_count,
    ...
    project_deficit = 1.0 - project_coverage
...
def calculate_technical_requirement_deficit(
    ...
    for project in data.projects:
        ...
        result = calculate_project_technical_coverage(
            data=data,
            project_id=project_id,
            team=team,
        )
    ...
    return {
        "technical_requirement_deficit": overall_deficit,
        "technical_requirement_coverage": 1.0 - overall_deficit,
...
def calculate_preference_dissatisfaction(
    ...
    for project_id, team in assignment.items():
        for student_id in team:
            student_project_lookup[student_id] = project_id

    for student in data.students:
        student_id = student.student_id
        assigned_project = student_project_lookup[student_id]
        ranked_projects = preference_lookup.get(
            student_id,
            [],
        )
        ...
        elif assigned_project in ranked_projects:
            rank = ranked_projects.index(
                assigned_project
            ) + 1
    ...
    return {
        "preference_dissatisfaction": overall_dissatisfaction,
        "preference_satisfaction": 1.0 - overall_dissatisfaction,
...
def evaluate_assignment(
    ...
    return {
        "technical_requirement_deficit": technical_result[
            "technical_requirement_deficit"
        ],
        "technical_requirement_coverage": technical_result[
            "technical_requirement_coverage"
        ],
        "preference_dissatisfaction": preference_result[
            "preference_dissatisfaction"
        ],
        "preference_satisfaction": preference_result[
            "preference_satisfaction"
        ],
        "project_results": technical_result[
            "projects"
        ],
        "student_preference_results": preference_result[
            "students"
        ],
    }